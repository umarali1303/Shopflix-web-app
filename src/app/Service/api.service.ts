...
export class ApiService {
  ...
  searchProductsPaginate(theKeyword:string):Observable<any> {
    const myheader = new HttpHeaders().set('AUTH_TOKEN', this.getToken());
      const searchUrl = `http://localhost:8087/user/search?productname=${theKeyword}`;
      return this.http.get<any>(searchUrl,{headers:myheader});
}
